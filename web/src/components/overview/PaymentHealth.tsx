...
function Delta({ segment }: { segment: Segment }) {
  ...
  return (
    <span className={`mono text-[11.5px] ${SEVERITY_CLASS[severity]}`}>
    ...
    <div className="divide">
      {board.methods.map((entry) => {
        ...
        return (
          ...
          >
            <div className="text-[13px] font-medium uppercase tracking-[0.04em]">
              {entry.method}
            </div>
            {measured ? (
              <>
  ...
  const lane = (
    <div className="lane row-hover">
      <div className="flex items-center gap-2 truncate">
        ...
        <span className={`mono truncate text-[12px] ${inIncident ? "crit" : "mid"}`}>
          {node.instrument}
        </span>
      </div>
      <DeviationBar segment={segment} />
      <div className="fig-md text-right text-[13px]">{percent(segment.rate)}</div>
      <div className="text-right">
