    ...
    <article className="px-2 py-4 first:pt-0">
      ...
      <div className="mt-4 grid grid-cols-2 gap-x-6 gap-y-4 sm:grid-cols-3 lg:grid-cols-6">
        <Field label="Opened">
          ...
          <span className="dim mono text-[12px]">{elapsed(incident.opened_at, now)}</span>
        </Field>
        ...
        <Field label="Segment rate">
          {segment ? (
            ...
            <span className="dim mono text-[12px]">below floor this window</span>
          )}
        </Field>
        <Field label="Cases">
          <span className="mono">{count(incident.cases)}</span>{" "}
          <span className="dim mono text-[12px]">
        ...
        <Field label="Time to detect">
          {detectedIn === null ? (
            <span className="dim mono text-[12px]">-</span>
          ) : (
      ...
      {failed && (
        <div className="alert mt-4" role="alert">
          <div className="flex flex-wrap items-baseline gap-x-3">
            ...
            <span className="mono text-[12px]">{plannerError}</span>
          </div>
