      ...
      {entries.map((entry) => (
        <li key={entry.seq} className="py-2">
          <div className="flex items-baseline gap-2">
            <span className="num w-10 shrink-0 text-right text-[length:var(--fs-small)] text-[color:var(--fg-3)]">
              {entry.seq}
            </span>
            <span className="num w-20 shrink-0 text-[length:var(--fs-small)] text-[color:var(--fg-2)]">{timeOnly(entry.ts)}</span>
            <Badge tone={toneFor(entry.kind)}>{entry.kind}</Badge>
            <span className="text-[length:var(--fs-small)] text-[color:var(--fg)]">{summarise(entry)}</span>
          </div>
          <div className="ml-32">
