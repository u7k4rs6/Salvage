  ...
  return (
    <div className="panel p-4">
      ...
      <div className="mt-3 flex items-baseline gap-3">
        ...
        <span className="mono dim text-[14px]">not recorded</span>
      </div>
    ...
    <details className="mt-4 group">
      ...
      <div className="panel mt-2 p-4">
        <div className="kv">
          ...
          <span className="mono mid text-[13px]">{evidence.segment_key}</span>
          <span className="lbl">Scope</span>
          <span className="mono mid text-[13px]">{evidence.affected_scope.join(", ") || "-"}</span>
          ...
          <span className="mono mid text-[13px]">{count(evidence.attempts)}</span>
          ...
          <span className="mono mid text-[13px]">{evidence.excess_failures.toFixed(1)}</span>
          <span className="lbl">Share of volume</span>
          <span className="mono mid text-[13px]">
            {percent(evidence.share_of_merchant_volume)}
          </span>
          <span className="lbl">Trend</span>
          <span className="mono mid text-[13px]">{evidence.trend}</span>
          ...
          <span className="mono mid text-[13px]">{evidence.minutes_since_onset}</span>
        </div>
      ...
      <div className="divide mt-1.5">
        ...
        {rows.map(([name, share]) => (
          <div key={name} className="flex items-baseline justify-between gap-3 py-1.5">
            <span className="mono mid truncate text-[13px]">{name}</span>
