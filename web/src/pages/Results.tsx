  ...
  return (
    <Table columns={["scenario", "at-risk orders", ...policies]}>
      {scenarios.map((scenario) => {
        ...
        return (
          <tr key={scenario} className="border-b border-neutral-100">
            ...
            {policies.map((policy) => {
              ...
              if (!row) return <td key={policy} className="cell-pad text-neutral-500">not run</td>;
              return (
  ...
  return (
    ...
    >
      {run.aggregates.map((row) => (
        <tr key={`${row.scenario}-${row.policy}`} className="border-b border-neutral-100">
          ...
          <td className="cell-pad num text-right">
            {row.violations === 0 ? (
              ...
              <span className="text-red-700">{row.violations}</span>
            )}
          </td>
        </tr>
      ))}
              ...
              right={
                <label className="text-xs text-neutral-700">
                  ...
                  <select
                    ...
                    className="num border border-neutral-300 px-2 py-1 text-xs"
                  >
                    {list.runs.map((item) => (
                ...
                {(data.diagnosis.misses ?? []).length > 0 && (
                  <ul className="mt-2 space-y-0.5">
                    {data.diagnosis.misses.map((miss: string, index: number) => (
                      <li key={index} className="num text-[11px] text-neutral-600">
                        {miss}
                      </li>
                    ))}
                  </ul>
                ...
                <div className="h-56">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={data.sensitivity.rows ?? []}>
                      ...
                      <XAxis dataKey="scale" tick={{ fontSize: 11 }} stroke="#737373" />
                      <YAxis
                        tick={{ fontSize: 11 }}
                        stroke="#737373"
                        width={70}
            ...
            {data.fault_injection && (
              ...
              >
                <div className="grid grid-cols-2 gap-3 text-xs lg:grid-cols-4">
                  ...
                  <div>
                    <div className="text-neutral-600">refused</div>
                    <div className="num text-lg text-green-700">
                      {data.fault_injection.refused}
                    </div>
                  </div>
                  <div>
                    <div className="text-neutral-600">ledgered</div>
                    <div className="num text-lg">{data.fault_injection.ledgered}</div>
                  </div>
                  <div>
                    <div className="text-neutral-600">unrefused</div>
                    <div
