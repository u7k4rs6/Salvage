...
REQUIRED_NOTES = DOCS / "required_notes"


def test_results_still_carries_every_required_note():
    """Limitations added to the generated results file by hand, and the guard that they survive.

    `docs/RESULTS.md` is written by `salvage eval report`. A limitation added to it by hand lives
    until the next regeneration and then vanishes silently, which is the failure `check_freshness`
    exists to catch in the other direction: a document that still looks complete after something a
    reader would act on has left it. Each `*.note.md` file under `docs/required_notes` is the
    canonical text of one such note. This fails by name when one is missing.

    The permanent fix for any note here is to add its text to the `items` list in `_limitations()`
    and delete the file. That is a change inside `salvage/`, deferred while the build is frozen.
    """
    text = _read(RESULTS)
    notes = sorted(REQUIRED_NOTES.glob("*.note.md"))
    assert notes, f"{REQUIRED_NOTES} has no *.note.md files, so this guard is guarding nothing"

    missing = [note.name for note in notes if note.read_text(encoding="utf-8").strip() not in text]
    assert missing == [], (
        f"{RESULTS} no longer carries {missing}. It was probably regenerated by "
        f"`salvage eval report`, which renders `_limitations()` and does not know about these. "
        f"Paste the file's contents back into the limitations section, or add the text to the "
        f"`items` list in `salvage/eval/report.py` and delete the note file. "
        f"See {REQUIRED_NOTES / 'README.md'}."
    )
